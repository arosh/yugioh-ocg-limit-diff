...
test(async t => {
  ...
  t.is(text, '2019/01/01');
  const href = await browser.getAttribute(selector, 'href');
  const url =
    'http://yugioh-wiki.net/index.php?%A5%EA%A5%DF%A5%C3%A5%C8%A5%EC%A5%AE%A5%E5%A5%EC%A1%BC%A5%B7%A5%E7%A5%F3%2F2019%C7%AF1%B7%EE1%C6%FC';
  t.is(href, url);
});
...
test(async t => {
  ...
  const text = await browser.getText(selector);
  t.is(text, '2018/10/01');
  const href = await browser.getAttribute(selector, 'href');
  const url =
    'http://yugioh-wiki.net/index.php?%A5%EA%A5%DF%A5%C3%A5%C8%A5%EC%A5%AE%A5%E5%A5%EC%A1%BC%A5%B7%A5%E7%A5%F3%2F2018%C7%AF10%B7%EE1%C6%FC';
  t.is(href, url);
});
